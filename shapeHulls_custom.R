# Custom Shape hulls function for when 
# have one group with only 1 member
# KWolcott 3 May

# Modified from https://github.com/cran/geomorph/blob/master/R/shapeHulls.r
...
shapeHulls1 <- function(x, groups = NULL, group.cols = NULL, 
                       group.lwd = NULL, group.lty = NULL){
  y <- as.matrix(x$PC.points[2:length(pca.plot$PC.points[,1]),]) #skip rehmii (seed 1) for polygons bc only 1 point
  groups <- groups[2:length(pca.plot$PC.points[,1])] #skip rehmii for polygons bc only 1 point
